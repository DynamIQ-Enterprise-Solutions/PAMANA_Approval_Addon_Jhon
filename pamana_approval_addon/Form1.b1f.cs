...
namespace pamana_approval_addon
{
    ...
    {    
        ...
        public Form1()
        {
            Logger.WriteToFile("OK", "Form1", "Test");
            LoadGrid();
        ...
        public override void OnInitializeComponent()
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.WriteToFile("ERROR", "OnInitializeComponent", ex.Message);     //ADD ERROR LOG [15JUN2026]
                Application.SBO_Application.StatusBar.SetText(ex.ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error); //ADD ERROR LOG [15JUN2026]               
        ...
        private void Button0_PressedAfter(object sboObject, SAPbouiCOM.SBOItemEventArg pVal)
        {
            ...
            {
                Logger.WriteToFile("ERROR", "Button0_PressedAfter", ex.Message);   //ADD ERROR LOG [15JUN2026]
                Application.SBO_Application.StatusBar.SetText(ex.ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error); //ADD ERROR LOG [15JUN2026]
            }
            finally
            {
        ...
        private void ApproveDocument(int WddCode, int draftKey)
        {
            ...
            try
            {
                ...
                SaveDraftToDoc_2(draftKey);  //[15JUN2026]
        ...
        private void SaveDraftToDoc(int draftentry)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    oGetPr.DoQuery($"select A.DocEntry, A.DocNum, B.BeginStr,A.DocType from OPRQ A left join NNM1 B on A.Series = B.Series where A.DocEntry = '{Program.oGlobalCompany.GetNewObjectKey()}'");
                    ...
                    if (oGetAttachPath.RecordCount > 0 && oGetReportName.RecordCount > 0 && oGetPr.RecordCount > 0)
                    {
                        ...
                        if (oGetPr.Fields.Item("DocType").Value.ToString().Equals("S"))
                        {
                            Attachmentss.GeneratePDF($"{oGetAttachPath.Fields.Item("Name").Value.ToString()}", $"{oGetAttachPath.Fields.Item("Name").Value.ToString()}", oGetPr.Fields.Item("DocEntry").Value.ToString(), oGetReportName.Fields.Item("Name").Value.ToString()+"S");
            ...
            try
            {
                ...
                if (oDraft.GetByKey(draftentry))
                {
                    ...
                    if (errmsg != 0)
                    {
                        ...
                        //Application.SBO_Application.StatusBar.SetText(Program.oGlobalCompany.GetLastErrorDescription().ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);   //ORIG
        ...
        private void Button0_PressedBefore(object sboObject, SAPbouiCOM.SBOItemEventArg pVal, out bool BubbleEvent)
        {
            ...
            {
                Logger.WriteToFile("ERROR", "Button0_PressedBefore", ex.Message);   //ADD ERROR LOG [15JUN2026]
                Application.SBO_Application.StatusBar.SetText(ex.ToString(), SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Error);  //ADD ERROR LOG [15JUN2026]
            }
